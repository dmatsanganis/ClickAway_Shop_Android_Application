...
import com.dmatsanganis.clickawayapp.RecyclerViewAdapters.TabletRecyclerViewAdapter;
...
public class TabletsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //List that will contain all tablets
        ...
        TabletRecyclerViewAdapter adapter = new TabletRecyclerViewAdapter(tablets);
        recyclerView.setAdapter(adapter);

        //Get all tablets from firebase and notify the adapter of the recycler view
        mFirebaseDatabase = FirebaseDatabase.getInstance();
        myRef = mFirebaseDatabase.getReference();
        myRef.child("Products").child("Tablets")
                .addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        tablets.clear();
                        for (DataSnapshot tablet_snapshot : snapshot.getChildren()){
                            Tablet tablet = tablet_snapshot.getValue(Tablet.class);
                            tablets.add(tablet);
                        }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REC_RESULT && resultCode == RESULT_OK){
            ...
            if(matches.get(0).toLowerCase().contains("select") || (matches.get(0).toLowerCase().contains("open") || matches.get(0).toLowerCase().contains("επιλογή") || matches.get(0).toLowerCase().contains("άνοιγμα"))) {
                ...
                if (!numberOnly.equals("") && Integer.parseInt(numberOnly) <= tablets.size() && Integer.parseInt(numberOnly) > 0){
